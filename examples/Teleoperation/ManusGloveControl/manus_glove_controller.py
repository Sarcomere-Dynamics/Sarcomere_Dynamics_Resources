...
class ManusGloveController:
    ...
    def _initialize_api(self):
        ...
        if self.robot_config.config.robots.left_hand_robot.robot_connected:
            self.artus_api = self.robot_config.return_api(
                robot_cfg=self.robot_config.config.robots.left_hand_robot, logger=logger
            )

        # Check and print configuration for right hand robot
        if self.robot_config.config.robots.right_hand_robot.robot_connected:
            self.artus_api = self.robot_config.return_api(
                robot_cfg=self.robot_config.config.robots.right_hand_robot,
                logger=logger,
            )
    ...
    def start_streaming(self):
        ...
        while True:
            try:
                ...
                joint_angles_right = (
                    self.hand_tracking_data.get_right_hand_joint_angles()
                )
