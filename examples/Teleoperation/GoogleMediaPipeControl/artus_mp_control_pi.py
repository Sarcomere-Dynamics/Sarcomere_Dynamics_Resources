...
# ============================================================
#  CAMERA SELECTION (RASPBERRY PI CAMERA, HEADLESS)
# ============================================================

def get_pi_camera():
    """
    Open the Raspberry Pi camera using Picamera2 for headless use.
    Assumes the camera is exposed and Picamera2 is installed.
    """
    if Picamera2 is None:
        raise ImportError(
            "Picamera2 is not installed. Install with:\n"
            "  sudo apt install python3-picamera2\n"
            "or follow the Raspberry Pi documentation for Picamera2."
        )

    print("Opening Raspberry Pi camera with Picamera2 ...")
    picam2 = Picamera2()
...
def calibrate_finger_lengths(cap, detector, target_samples=50):
    ...
    - Automatically collects `target_samples` frames while the hand is in view.
    - No GUI windows or keypresses required.
    """
    print("\n=== CALIBRATION MODE (HEADLESS) ===")
    print("Ensure your RIGHT hand is flat, fingers extended, facing the camera.")
    print(f"Collecting {target_samples} samples after pressing 'c' ...")

    while True:
        if input("Press 'c' to start collecting samples: ") == 'c':
            break
...
def main():
    ...
    # Use Raspberry Pi camera in headless mode
    cap = get_pi_camera()
    ...
    while True:
        ...
        if result.hand_landmarks and result.handedness:
            ...
            # In headless mode, we do not display the frame.
            # draw_finger_angles(frame, mapped_angle_dict)  # Optional: keep for debugging if you later add display.

    cap.release()
    # No GUI windows were created in headless mode, so nothing to destroy.
