...
#include<ws2tcpip.h>  //Additional networking headers
...
constexpr unsigned long long int MILLISECONDS_BETWEEN_TEMPORARY_SKELETONS_UPDATE = 10;
...
protected:
	...
	// ---- Custom functions ----
	void SendErgonomicsData();
    bool IsFingerTipNode(const SkeletonNode& node);
    void SendFingerTipData();
    void ExtractFingerTipPoseData();

	// --------------------------

	void TCP_Send(const std::string& hand_data);
