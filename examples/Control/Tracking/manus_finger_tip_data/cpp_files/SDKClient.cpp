...
#define JOINT_POSITION_DATA 0 // Set to 1 to use joint position data, 0 to use fingertip data
#define FINGER_TIP_DATA !JOINT_POSITION_DATA

#define FINGER_TIP_START_ID 0  // Replace with the actual starting ID for fingertip nodes
#define FINGER_TIP_END_ID 24    // Replace with the actual ending ID for fingertip nodes
...
{
	// print data size
	// std::cout << "Data size: " << data.size() << std::endl; // output: 2500 bytes
...
{
	t_tcp_client.conn("127.0.0.1", 65432);
...
{
	// print to make sure we are getting the right data
	// std::cout << "Skeleton stream callback called" << std::endl;
	if (s_Instance)
	{
...
void SDKClient::TCP_Send(const std::string& hand_data)
{
	// Implementation of TCP send here
	// This is where you would connect and send `hand_data` to the server
	std::cout << hand_data << std::endl;
	t_tcp_client.send_data(hand_data);
...
void SDKClient::SendErgonomicsData()
{
	...
	const std::string t_FingerNames[NUM_FINGERS_ON_HAND] = { "thumb", "index", "middle", "ring", "pinky" };
	int t_DataOffset = 0;
	std::stringstream hand_data_array;
	hand_data_array << "L[";

	// Assuming m_LeftGloveErgoData and m_RightGloveErgoData are defined and populated
	if (m_LeftGloveErgoData.id == m_FirstLeftGloveID)
	{
		for (unsigned int t_FingerNumber = 0; t_FingerNumber < NUM_FINGERS_ON_HAND; t_FingerNumber++)
		{
			std::stringstream ss;
			ss << t_FingerNames[t_FingerNumber] << ": ";
			// Append joint angles for each finger
			for (int j = 0; j < 4; j++) { // Assuming there are 4 joints per finger including the wrist
				ss << RoundFloatValue(m_LeftGloveErgoData.data[t_DataOffset + j], 2) << " ";
				hand_data_array << RoundFloatValue(m_LeftGloveErgoData.data[t_DataOffset + j], 2) << " ";
			}
			t_DataOffset += 4; // Move to the next set of joint angles
			// TCP_Send(ss.str()); // Send the data
		}
		hand_data_array << "]";
		TCP_Send(hand_data_array.str());
	}
	else
	{
		spdlog::info(" ...No Left Glove Data...");
	}

	// Reset data offset for right glove
	t_DataOffset = 20;
	// clear hand data array
	hand_data_array.str("");
	hand_data_array << "R[";
	if (m_RightGloveErgoData.id == m_FirstRightGloveID)
	{
		for (unsigned int t_FingerNumber = 0; t_FingerNumber < NUM_FINGERS_ON_HAND; t_FingerNumber++)
		{
			...
			// Append joint angles for each finger
			for (int j = 0; j < 4; j++) { // Assuming there are 4 joints per finger including the wrist
				ss << RoundFloatValue(m_RightGloveErgoData.data[t_DataOffset + j], 2) << " ";
				hand_data_array << RoundFloatValue(m_RightGloveErgoData.data[t_DataOffset + j], 2) << " ";
			}
			t_DataOffset += 4; // Move to the next set of joint angles
			// TCP_Send(ss.str()); // Send the data
		}
	...
	{
		spdlog::info(" ...No Right Glove Data...");
	}
}

void SDKClient::ExtractFingerTipPoseData()
{
    if (m_Skeleton == nullptr || m_Skeleton->skeletons.empty())
    {
        spdlog::info("No skeleton data available.");
        return;
    }

    const auto& skeleton = m_Skeleton->skeletons[0]; // Assuming the first skeleton is the hand
    for (const auto& node : skeleton.nodes)
    {
        // Check if the node is a fingertip
        if (IsFingerTipNode(node))
        {
            // Log the fingertip position
            spdlog::info("Fingertip ID {}: Position ({:.2f}, {:.2f}, {:.2f})",
                         node.id, // Use the node's ID
                         node.transform.position.x, // X-coordinate of the position
                         node.transform.position.y, // Y-coordinate of the position
                         node.transform.position.z  // Z-coordinate of the position
            );
        }
    }
}
void SDKClient::SendFingerTipData()
{
	// Add static time tracking
	static std::chrono::high_resolution_clock::time_point s_LastSendTime = std::chrono::high_resolution_clock::now();
    ...
    if (m_RawSkeleton == nullptr || m_RawSkeleton->skeletons.empty())
    {
        spdlog::info("No raw skeleton data available.");
        return;
    }

    const auto& skeleton = m_RawSkeleton->skeletons[0];
    
    // Debug: Print total number of nodes
    spdlog::info("Total nodes in skeleton: {}", skeleton.nodes.size());
    
    std::stringstream fingertipData;
    fingertipData << (m_FirstLeftGloveID != 0 ? "L" : "R") << "_FingerTips[";

    bool foundAnyFingerTips = false;  // Debug: Track if we find any fingertips

    for (size_t j = 0; j < skeleton.nodes.size(); j++)
    {
        const auto& node = skeleton.nodes[j];
        
        // Debug: Print all node IDs
        spdlog::info("Node {} has ID: {}", j, node.id);
        
        if (IsFingerTipNode(node))
        {
            foundAnyFingerTips = true;  // Debug: Mark that we found a fingertip
            // Send both position and rotation data
            // fingertipData << "Node" << j << ": ID=" << node.id
			fingertipData << "Node ID: " << node.id
                         << " Pos=(" 
    ...
    // Debug: Print whether we found any fingertips
    if (!foundAnyFingerTips) {
        spdlog::info("No fingertip nodes found! Check FINGER_TIP_START_ID ({}) and FINGER_TIP_END_ID ({})", 
            FINGER_TIP_START_ID, FINGER_TIP_END_ID);
    }

    // Log the data we're trying to send
    spdlog::info("Attempting to send: {}", fingertipData.str());
    
    // Try to send the data
    TCP_Send(fingertipData.str());
}

bool SDKClient::IsFingerTipNode(const SkeletonNode& node)
{
    // Debug: Print node ID as we check it
    spdlog::info("Checking node ID {} against range {}-{}", 
        node.id, FINGER_TIP_START_ID, FINGER_TIP_END_ID);
    
    return node.id >= FINGER_TIP_START_ID && node.id <= FINGER_TIP_END_ID;
}
...
void SDKClient::PrintRawSkeletonData()
{
	...
	// print raw skeleton finger tip position and orientation data

#ifdef PRINT_DATA
    for (size_t i = 0; i < m_RawSkeleton->skeletons.size(); i++)
    {
        const auto& skeleton = m_RawSkeleton->skeletons[i];
        spdlog::info("Raw Skeleton Structure:");
        spdlog::info("Number of skeletons: {}", m_RawSkeleton->skeletons.size());
        spdlog::info("First skeleton:");
        spdlog::info("- Glove ID: 0x{:X}", skeleton.info.gloveId);
        spdlog::info("- Number of nodes: {}", skeleton.info.nodesCount);

        // Fix: Change the array access syntax
        // Print node information
        for (size_t j = 0; j < skeleton.nodes.size(); j++)
        {
            // Fix: Access nodes directly from skeleton
            const auto& node = skeleton.nodes[j];
            spdlog::info("Node {}: ID {}, Position: ({:.2f}, {:.2f}, {:.2f})",
                j,
                node.id,
                node.transform.position.x,
                node.transform.position.y,
			...
			spdlog::info("Node {}: Orientation: ({:.2f}, {:.2f}, {:.2f}, {:.2f})",
                j,
                node.transform.rotation.w,
                node.transform.rotation.x,
                node.transform.rotation.y,
